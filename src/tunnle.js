var tunnlejs = (function () {
    ...
    var createTunnle = function (outerSurface, innerSurface) {
            ...
            var closure = createClosures(outerSurface.closureSets, innerSurface.closureSets);
    ...
    var isInHole = function (holes,x,y) {
        for(var i = 1;i< holes.length;i++) {
            var hole = holes[i];
            if(x>=hole.left && x<=hole.right && y>=hole.bottom && y<=hole.top){
                return true;
            } 
        }
        return false;
    ...
    var fillHoles = function (holes,x,y, vertices) {
        for(var i = 0;i< holes.length;i++) {
            ...
            if(i==0){
                if(x>=holes[i].left && x<=holes[i].right && y>=holes[i].bottom && y<=holes[i].top){
                    if(x==holes[i].left) holes[i].leftVertices.unshift(vertices[0]);
                    if(y==holes[i].bottom) holes[i].bottomVertices.push(vertices[2]);
                    if(x==holes[i].right) holes[i].rightVertices.push(vertices[3]);
                    if(y==holes[i].top) holes[i].topVertices.unshift(vertices[1]);
                }
            } else {
                if(x>=holes[i].left && x<=holes[i].right && y>=holes[i].bottom && y<=holes[i].top){
                    if(x==holes[i].left) holes[i].leftVertices.push(vertices[1]);
                    if(y==holes[i].bottom) holes[i].bottomVertices.unshift(vertices[0]);
                    if(x==holes[i].right) holes[i].rightVertices.unshift(vertices[2]);
                    if(y==holes[i].top) holes[i].topVertices.push(vertices[3]);
                }
            }
        }
    }
    ...
    var createClosureSets = function (holes) {
        var closureSets = [];
        for(var i = 0;i< holes.length;i++) {
            if(i==0)closureSets.push(holes[i].bottomVertices.concat(holes[i].rightVertices).concat(holes[i].topVertices).concat(holes[i].leftVertices));
            if(i>0)closureSets.push(holes[i].leftVertices.concat(holes[i].topVertices).concat(holes[i].rightVertices).concat(holes[i].bottomVertices));
